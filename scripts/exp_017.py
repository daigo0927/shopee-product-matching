...
class FeatureFactory:
    def __init__(self, target_size, tfidf, encoder_weights):
...
class DatasetFactory:
    ...
    def build(self, inputs, image_aug=None, batch_size=4, is_training=False):
        ...
        ds_embed = tf.data.Dataset.from_tensor_slices(inputs.embeds)
...
def build_model(image_size,
    ...
    similarity = CircleLossCL(n_classes, **circle_loss)
...
class EmbeddingF1(callbacks.Callback):
    def __init__(self,
                 ds: tf.data.Dataset,
                 df: pd.DataFrame,
                 patience: int,
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        for thr in np.arange(0.1, 1.0, 0.1):
            ...
            current = max(current, f1)
            
        df = pd.DataFrame(scores, columns=['threshold', 'f1score'])
        print('Current val f1:', current)
...
def train(config, logdir):
    ...
    for cv, (train_idx, val_idx) in enumerate(splits):
        ...
        inputs_train = feat_factory.build(df_t)
        inputs_val = feat_factory.build(df_v)
        ...
        recorder = EmbeddingF1(ds_val, df_v, **train_config.callback)
        model.fit(
            ds_train,
        ...
        f1_best = recorder.best
        df_best = recorder.df_best
        print(f'Best f1score: {f1_best:.4f}')

        model_dir = f'{logdir}/models'
        if not os.path.exists(model_dir):
            os.makedirs(model_dir)
        model.save_weights(f'{model_dir}/cv{cv}-{f1_best:.4f}')

        score_dir = f'{logdir}/scores'
        if not os.path.exists(score_dir):
            os.makedirs(score_dir)
        df_best.to_csv(f'{score_dir}/cv{cv}.csv', index=False)

        cv_scores.append(f1_best)

    df_cv = pd.DataFrame(cv_scores, columns=['f1'])
    df_cv.to_csv(f'{score_dir}/total.csv', index=False)
