...
def rearrange(image_inputs, tokens, tfidf, embed):
    image, label_id = image_inputs
    input_ids = tokens['input_ids']
    attention_mask = tokens['attention_mask']
    tokens = [input_ids, attention_mask]
    return (image, *tokens, tfidf, embed, label_id), (label_id, 0)
...
class FeatureFactory:
    def __init__(self, target_size, tokenizer, tfidf, encoder_weights):
...
class DatasetFactory:
    ...
    def build(self, inputs, image_aug=None, batch_size=4, is_training=False):
        ...
        ds = tf.data.Dataset.zip((ds_image, ds_tokens, ds_tfidf, ds_embed))
...
def build_model(image_size,
    ...
    feat = tf.concat([image_feat, bert_feat, tfidf_feat, embed], axis=-1)
    ...
    model = tf.keras.Model(inputs=[image, *tokens, tfidf, embed, label],
                           outputs=[logits, feat])
    return model
